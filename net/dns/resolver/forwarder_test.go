...
func runDNSServer(tb testing.TB, response []byte, onRequest func(bool, []byte)) (port uint16) {
	...
	if tcpLn == nil || udpLn == nil {
		tb.Fatal("failed to listen on same port for TCP/UDP")
	}

	port = uint16(tcpLn.Addr().(*net.TCPAddr).Port)

	tb.Cleanup(func() { tcpLn.Close() })
	tb.Cleanup(func() { udpLn.Close() })
...
func TestForwarderTCPFallback(t *testing.T) {
	envknob.Setenv("TS_DEBUG_DNS_FORWARD_SEND", "true")
	...
	fwd := newForwarder(t.Logf, netMon, nil, &dialer)
